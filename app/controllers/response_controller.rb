class ResponseController < ApplicationController
  ...
  #Function to find recipients for email When a FeedBack is given

  def emailOnFeedback(revieweeId)
  ...
  #Function to find recipients for email When Metareview is done
  ...
  #Function to find recipients for email When Team Mate review is done

  def emailOnTeammateReview(revieweeId)
    participant = Participant.find_by_id(revieweeId).user_id
    user=User.find_by_id(participant)
    asstId = Participant.find_by_id(revieweeId).parent_id
    assignment=Assignment.find_by_id(asstId).name
  ...
  #Function to find recipients for email When Work product review is done
...
class ResponseController < ApplicationController
  ...
  def create
    ...
    reviewType=ResponseMap.find_by_id(@map.id).type
    revieweeId=ResponseMap.find_by_id(@map.id).reviewee_id

    if (reviewType.to_s=="FeedbackResponseMap")
      emailOnFeedback(revieweeId)
    end

    if (reviewType.to_s=="MetareviewResponseMap")
      emailOnMetareview(revieweeId)
    end

    if (reviewType.to_s=="TeammateReviewResponseMap")
      emailOnTeammateReview(revieweeId)
    end

    if (reviewType.to_s=="TeamReviewResponseMap")
      emailOnReview(revieweeId)
    end
  end
  ...
  def custom_create
    ...
    puts "We are in create function"
    reviewType=ResponseMap.find_by_id(@response.id).type
